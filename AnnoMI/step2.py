...
import json
import argparse
from prompts.turn_labeling_prompt import get_prompt
import random
import re
...
parser.add_argument('--step1_path', type=str, default = "/home/jihyunlee/MI/AnnoMI/generated/step1_annomi.json")
parser.add_argument('--actions_path', type=str, default = "/home/jihyunlee/MI/source/action.json")
parser.add_argument('--num_to_generate', type=int, default=1) # number of intake forms to generate
...
def make_action_description(attitude_actions):
    action_descriptions = "\n".join(
            [f"- {k}: {v['description']}" for k, v in attitude_actions.items()]
        )
...
if __name__ == "__main__":
    ...
    save_temp = f"./temp/step2.jsonl"
    save_result = f"batch_output/step2.jsonl"
    save_processed = f"{args.save_folder}/step2_annomi.json"
    ...
    attitude_actions = json.load(open(args.actions_path))
    ...
    for idx in range(num_to_generate):
        ...
        for turn_idx, turn in enumerate(dialogue):
            if turn["interlocutor"] != "client": continue
